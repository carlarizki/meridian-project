...
function ExecutiveAha({ go, portfolio }: { go: (id: ScreenId) => void; portfolio: ReturnType<typeof calculatePortfolio> }) {
  return <>
    <section className="welcome-band">
      <div className="welcome-copy"><Badge tone="info"><Sparkles className="size-3" /> DIRECTOR OF HUMAN CAPITAL · 90-DAY PROOF POINT</Badge><h1>Turn disrupted work into <span>defensible people pathways</span></h1><p>See how 4,200 roles’ worth of changing work can become evidence-backed redeployment, reskilling, and assessment pathways—without automatic employment decisions.</p><Button onClick={() => go("exposure")}>Trace the 4,200 signal <ArrowRight className="size-4" /></Button></div>
    ...
    <section className="aha-flow" aria-label="Workforce transition journey"><div><span>Enterprise scope</span><strong>52,000</strong><small>employees</small></div><ArrowRight/><div><span>Day-30 priority</span><strong>6,000</strong><small>Field Metering & Manual Operations</small></div><ArrowRight/><div className="critical"><span>Work disruption</span><strong>{formatCount(portfolio.highExposureWork)}</strong><small>roles’ worth of work · 18 months</small></div><ArrowRight/><button onClick={() => go("mobility")}><span>Actionable portfolio</span><strong>{formatCount(portfolio.projectedLandings)}</strong><small>projected landing paths</small></button></section>
    <div className="metric-grid four"><Metric icon={Users} label="Enterprise Workforce" value="52,000" note="Case-brief scope"/><Metric icon={Network} label="Priority Job Family" value="6,000" note="Simulated cohort" tone="amber"/><Metric icon={Activity} label="Work at High Exposure" value={formatCount(portfolio.highExposureWork)} note="70% automatable work" tone="red"/><Metric icon={ShieldCheck} label="Decision Principle" value="Human" note="Review required" tone="green"/></div>
  </>;
}

function Workforce({ region, setRegion, go, portfolio }: { region: string; setRegion: (v: string) => void; go: (id: ScreenId) => void; portfolio: ReturnType<typeof calculatePortfolio> }) {
  const multiplier = region === "All Units" ? 1 : 0.16;
  return <><ExecutiveAha go={go} portfolio={portfolio}/><PageHeader title="Workforce Overview" subtitle="Enterprise context with a focused Day-30 intervention cohort."><SelectControl value={region} onChange={setRegion} label="Regional unit" options={["All Units", "Regional 1", "Regional 2", "Regional 3", "Regional 4", "Regional 5", "Regional 6", "Head Office"]}/></PageHeader>
    ...
    <section className="panel"><div className="panel-head"><div><h2>AI exposure distribution</h2><p>Enterprise estimate, weighted by task composition</p></div></div><div className="donut-layout"><Donut/><div className="legend"><p><i className="dot bad"/><span>High exposure</span><strong>25% · 13,000</strong></p><p><i className="dot warn"/><span>Medium exposure</span><strong>40% · 20,800</strong></p><p><i className="dot good"/><span>Low exposure</span><strong>35% · 18,200</strong></p></div></div><div className="insight"><BrainCircuit className="size-4"/><p><strong>Priority signal:</strong> {formatCount(portfolio.highExposureWork)} roles’ worth of work changes—not {formatCount(portfolio.highExposureWork)} automatic job losses.</p></div></section></div>
  </>;
}
...
function People({ inferred, setInferred }: { inferred: boolean; setInferred: (v: boolean) => void }) {
  const [view, setView] = useState<"directory" | "profile">("directory");
  return <><div className="view-switch"><Button variant={view === "directory" ? "default" : "outline"} onClick={() => setView("directory")}><Users className="size-4"/> Directory</Button><Button variant={view === "profile" ? "default" : "outline"} onClick={() => setView("profile")}><UserRound className="size-4"/> Evidence profile</Button><Badge tone="info">24 CURATED PROFILES · 6,000 COHORT MODEL</Badge></div>{view === "directory" ? <Directory/> : <Profile inferred={inferred} setInferred={setInferred}/>}</>;
}

function Exposure({ region, setRegion, scenario, setScenario, portfolio }: { region: string; setRegion: (v: string) => void; scenario: MeridianScenario; setScenario: (v: MeridianScenario) => void; portfolio: ReturnType<typeof calculatePortfolio> }) {
  return <><PageHeader title="AI Exposure Analysis" subtitle="Separate changing work from individual employment outcomes."><SelectControl value={region} onChange={setRegion} label="Regional unit" options={["All Units", "Regional 1", "Regional 2", "Regional 3", "Regional 4", "Regional 5", "Regional 6"]}/></PageHeader>
    <div className="metric-grid three"><Metric icon={Users} label="Priority Cohort" value="6,000" note="One job family"/><Metric icon={Activity} label="High-Exposure Work" value={formatCount(portfolio.highExposureWork)} note={`${scenario.exposureCutoff}% scenario cutoff`} tone="red"/><Metric icon={ShieldCheck} label="Lower-Exposure Work" value={formatCount(portfolio.lowerExposureWork)} note="Retained or augmented" tone="green"/></div>
    ...
    <section className="crisis-panel"><div className="crisis-kicker"><Activity className="size-4"/> CRITICAL WORKFORCE SIGNAL</div><h2>Field Metering &<br/>Manual Operations</h2><p>Smart-meter rollout and AI-assisted dispatch change routine task demand; the signal is about work, not an automated decision about people.</p><div className="crisis-ring"><div><strong>{scenario.exposureCutoff}%</strong><span>automatable<br/>in 18 months</span></div></div><label className="scenario-control"><span>Exposure scenario <strong>{scenario.exposureCutoff}%</strong></span><input aria-label="Exposure cutoff" type="range" min="60" max="80" step="5" value={scenario.exposureCutoff} onChange={(e) => setScenario({...scenario, exposureCutoff:Number(e.target.value)})}/></label><div className="crisis-stats"><div><span>Priority cohort</span><strong>6,000 people</strong></div><div><span>Work disrupted</span><strong>~{formatCount(portfolio.highExposureWork)}</strong></div></div><div className="crisis-note"><ShieldCheck className="size-4"/><span>Zero mass layoffs. Every individual outcome requires human review.</span></div></section></div>
  </>;
}
